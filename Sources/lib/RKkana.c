 ...
 */
static unsigned short hiragana[] = {
  /* 0x00 */
  0x0000,
  0x0000,
  0x0000,
...
RkCvtZen(unsigned char* zen, int maxzen, unsigned char* han, int maxhan)
{
  ...
  while (h < H) {
    ...
    if (hi == 0x8e) { /* hankaku katakana */
      ...
      code = (((unsigned long)h[0]) << 8) | ((unsigned long)h[1]);
      h += 2;
      byte = 2;
  ...
  while (z < Z) {
    ...
    switch (hi) {
      ...
      case 0xa3: /* eisuuji */
        ...
        if (0xb0 <= lo && lo <= 0xb9)
          code = (lo - 0xb0) + '0';
        else if (0xc1 <= lo && lo <= 0xda)
          code = (lo - 0xc1) + 'A';
        else if (0xe1 <= lo && lo <= 0xfa)
          code = (lo - 0xe1) + 'a';
        else {
          code = (hi << 8) | lo;
          byte = 2;
      ...
      default:
        if (hi == 0x8f) {
          ...
          code = (((unsigned long)z[0]) << 8) | ((unsigned long)z[1]);
          z += 2;
          byte = 2;
...
RkCvtKana(unsigned char* kana, int maxkana, unsigned char* hira, int maxhira)
{
  ...
  while (h < H) {
    ...
    if (hi == 0x8f) {
      ...
      byte = 2;
      /* hiragana U + " */
      dakuon = (h + 1 < H && ((((unsigned long)h[0]) << 8) |
                              ((unsigned long)h[1])) == 0xa1ab);
...
static unsigned kurai3new[] = {
  0,
  0xbdbd,
  0xc9b4,
  0xc0e9,
};
static unsigned kurai3old[] = {
  0,
  0xbdbd,
  0xc9b4,
  0xc0e9,
...
RkwCvtHira(wchar_t* dst, int maxdst, wchar_t* src, int srclen)
{
  ...
  len = CNvW2E(src, srclen, cbuf, CBUFSIZE);
  len = RkCvtHira((unsigned char*)cbuf2, CBUFSIZE, (unsigned char*)cbuf, len);
  if (len > 0) {
...
RkwCvtKana(wchar_t* dst, int maxdst, wchar_t* src, int srclen)
{
  ...
  len = CNvW2E(src, srclen, cbuf, CBUFSIZE);
  len = RkCvtKana((unsigned char*)cbuf2, CBUFSIZE, (unsigned char*)cbuf, len);
  if (len > 0) {
    cbuf2[len] = '\0';
    len = CANNA_mbstowcs(dst, cbuf2, maxdst);
  }
...
RkwCvtZen(wchar_t* dst, int maxdst, wchar_t* src, int srclen)
{
  ...
  len = CNvW2E(src, srclen, cbuf, CBUFSIZE);
  len = RkCvtZen((unsigned char*)cbuf2, CBUFSIZE, (unsigned char*)cbuf, len);
  if (len > 0) {
    cbuf2[len] = '\0';
    len = CANNA_mbstowcs(dst, cbuf2, maxdst);
  }
...
int
RkwMapRoma(struct RkRxDic* romaji,
           wchar_t* dst,
           int maxdst,
           wchar_t* src,
           int srclen,
