...
#define L4TOL(l4)                                                              \
  ((((((((unsigned long)((unsigned char)(l4)[0])) << 8) |                      \
       ((unsigned long)((unsigned char)(l4)[1])))                              \
      << 8) |                                                                  \
     ((unsigned long)((unsigned char)(l4)[2])))                                \
    << 8) |                                                                    \
...
RkwOpenRoma(char* romaji)
{
  ...
  if (rdic) {
    ...
    if (rdic->nr_nkey > 0) {
      rdic->nr_keyaddr = (unsigned char**)calloc((unsigned)rdic->nr_nkey,
...
#ifdef JAPANESE_SORT
    tmp_rdic =
      (struct romaRec*)calloc((unsigned)rdic->nr_nkey, sizeof(struct romaRec));
    if (!tmp_rdic) {
      free(rdic->nr_string);
      ...
      switch ((i < maxsrc) ? findRoma(rdic, m, src[i], i, 0) : 0) {
        case 0:
          ...
          if (m == match) { /* table ni nakatta tokino shori */
            ...
            if ((flags & RK_SOKON) && (match[1].start < rdic->nr_nkey) &&
                (2 <= maxsrc) && (src[0] == src[1]) && (i == 1)) {
...
int
RkMapPhonogram(struct RkRxDic* rdic,
               unsigned char* dst,
               int maxdst,
               unsigned char* src,
               int srclen,
               ...
               int* rule_id_inout)
{
  ...
  if (rdic) {
    if ((rdic->dic == RX_KPDIC || rdic->dic == RX_PTDIC) && rule_id_inout &&
        (lastrule = *rule_id_inout)) {
      if (!key) {
    ...
    for (i = 0; (flags & RK_FLUSH) || i < srclen; i++) {
      m[1] = m[0];
      m++;
      switch ((i < srclen) ? findRoma(rdic, m, src[i], i, flags & RK_IGNORECASE)
                           : 0) {
        case 0:
          while (--m > match && xkey(rdic, m->start, m - match))
            /* EMPTY */
          ...
          if (m == match) { /* テーブルになかった時の処理 */
            ...
            if ((rdic->dic == RX_RXDIC) && /* tt の救済(旧辞書用) */
                (flags & RK_SOKON) && (match[1].start < rdic->nr_nkey) &&
                (2 <= srclen) && (src[0] == src[1]) && (i == 1)) {
              kana = getTSU(rdic, flags);
              /* tsu ha jisho ni aru kao wo suru */
              byte = strlen((char*)kana);
              templen = 0;
