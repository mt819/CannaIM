              ...
              unsigned char* keybuff)
{
  ...
  if (mode &&
      mode->func((uiContext)0 /*dummy*/, mode, KEY_CHECK, 0 /*dummy*/, fnum)) {
      ...
      if (mode->flags & CANNA_KANJIMODE_TABLE_SHARED) {
        ...
        for (i = 0; i < 256; i++) {
          ...
          if (mode->keytbl[i] == CANNA_FN_UseOtherKeymap) {
            debug_message(
              "changeKeyfunc:\245\306\241\274\245\326\245\353"
...
static int
changeKeyOnSomeCondition(KanjiMode mode,
                         int key,
                         int fnum,
                         unsigned char* actbuff,
                         unsigned char* keybuff)
  ...
  if (key >= 0 && key < 255) {
    ...
    for (ep = extrafuncp; ep; ep = ep->next) {
      ...
      if (ep->keyword == EXTRA_FUNC_DEFMODE) {
        retval = changeKeyOnSomeCondition(
          ep->u.modeptr->emode, key, fnum, actbuff, keybuff);
        if (retval < 0) {
          return retval;
        }
      }
    ...
    for (i = 0; i < CANNA_MODE_MAX_REAL_MODE; i++) {
      ...
      if (mode && /* そのモードが存在するなら */
          mode->func(
            (uiContext)0 /*dummy*/, mode, KEY_CHECK, 0 /*dummy*/, fnum)) {
        /* 関数がそのモードで有効なら */
        if (!(mode->flags & CANNA_KANJIMODE_TABLE_SHARED)) {
          /* テーブルが共有されていないなら */
          if (mode->keytbl) { /* キーテーブルが存在すれば */
...
static unsigned int
createHashKey(unsigned char* data1, unsigned char data2, int which_seq)
{
...
#ifdef CODED_MESSAGE
  debug_message(
    "actFromHash:キーシケンスをみつけられませんでした。\n", 0, 0, 0);
#else
  debug_message(
    "actFromHash:\245\255\241\274\245\267\245\261\245\363\245\271"
...
regist_act_hash(unsigned char* tbl_ptr, unsigned char key, unsigned char* buff)
{
  ...
  hashKey = createHashKey(tbl_ptr, key, ACTHASHTABLESIZE);
  for (pp = &seq_hash[hashKey]; (p = *pp) != NULL; pp = &(p->next)) {
    if (p->to_tbl == tbl_ptr && p->as_key == key) {
      if (p->kinou_seq)
        free(p->kinou_seq);
...
specialen(unsigned char* block)
{
  ...
  debug_message(
    "specialen:\304\271\244\265\244\317%d\244\311\244\271\241\243\n", i, 0, 0);
  /* specialen:長さは%dどす。 */
  return i;
}
...
to_write_act(int depth, int keysize, int actsize, unsigned singleAct)
{
  if (depth == (keysize - 2)) {
    if (actsize > 1) {
      debug_message(
        "to_write_act:CANNA_FN_FuncSequence\244\307\244\271\241\243\n",
        0,
        0,
        0);
      /* です。 */
    ...
    if (actsize == 1) {
      debug_message(
        "to_write_act:singleAct%d\244\307\244\271\241\243\n", singleAct, 0, 0);
      /* です。 */
    ...
    debug_message(
      "to_write_act:CANNA_FN_UseOtherKeymap\244\307\244\271\241\243\n",
      0,
      0,
      0);
    /* です。 */
           ...
           int depth)
{
  ...
  debug_message(
    "regist_map:hashKey = %d \244\307\244\271\241\243\n", hashKey, 0, 0);
  /* です。 */
  for (pp = &otherMap[hashKey]; (p = *pp) != NULL; pp = &(p->next)) {
    if (p->key == keybuff[depth] && p->tbl == tbl) {
        ...
        if (*q == keybuff[depth + 1]) { /* 既に同じキーが存在した。 */
          ...
          *q = to_write_act(depth, keybuffsize, actbuffsize, actbuff[0]);
          ...
          if (*q == CANNA_FN_FuncSequence) {
            regist_act_hash(
              (unsigned char*)p->mode, keybuff[depth + 1], actbuff);
          }
          debug_message("regist_map:"
                        "\264\373\244\313\306\261\244\270\245\255\241\274\244"
                        "\254\302\270\272\337:q=%d\n",
                        *q,
                        0,
                        0);
          /* 既に同じキーが存在 */
      ...
      debug_message(
        "regist_map:\244\275\244\263\244\336\244\307\244\316"
    ...
    if (p->mode) {
      ...
      if (p->mode->keytbl) {
        ...
        p->mode->keytbl[1] =
          to_write_act(depth, keybuffsize, actbuffsize, actbuff[0]);
        debug_message(
          "regist_map:p->mode->keytbl[1]\244\317%d\244\307\244\271\241\243\n",
          p->mode->keytbl[1],
          0,
          0);
        /* は%dです。 */
        ...
        for (i = 0, new_sequence = p->mode->keytbl; i <= sequencelen; i++) {
          ...
          if (i % 2 == 1) {
            if (old_sequence[i] == CANNA_FN_UseOtherKeymap) {
              if (copyMultiSequence(
                    old_sequence[i - 1], old_map->mode, p->mode) < 0) {
                free(p->mode->keytbl);
                free(p->mode);
                free(p);
                *pp = NULL;
              ...
              regist_act_hash((unsigned char*)p->mode,
                              old_sequence[i - 1],
  ...
  for (i = 0; i <= sequencelen; i++) {
    if (i % 2 == 1) {
      ...
      if (sequence[i] == CANNA_FN_FuncSequence)
        remove_hash(
          (unsigned char*)map->mode, sequence[i - 1], ACTHASHTABLESIZE);
    }
  }
  debug_message(
    "\241\374\153\145\171\133\45\144\135\244\316\155\141\160\260"
