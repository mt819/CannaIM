...
RkCvtHan(unsigned char* han, int maxhan, unsigned char* zen, int maxzen)
{
  ...
  while (z < Z) {
    ...
    switch (hi) {
      ...
      case 0xa5: /* katakana */
        lo = *z++;
        if ((code = hankaku[lo]) && (lo <= (Wchar)(hi == 0xa4 ? 0xf3 : 0xf6))) {
          if (code >> 8) {
  ...
  for (i = 0, j = 0; src[i] != (Wchar)0 && j < dstlen && i < srclen; i++) {
    ...
    switch (codeset) {
      ...
      case 0x8080: /* Codeset 1, that is Kanji */
        ...
        sjise_kanjip[1] =
          ((sjise_kanjip[0] % 2)
             ? (sjise_kanjip[1] + 0x1f + ((sjise_kanjip[1] < 0x60) ? 0 : 1))
             : (sjise_kanjip[1] + 0x7e));
        sjise_kanjip[0] =
          ((sjise_kanjip[0] < 0x5f) ? ((sjise_kanjip[0] - 0x21) / 2 + 0x81)
...
static Wchar kurai3new[] = {
  0,
  0xbdbd,
  0xc9b4,
  0xc0e9,
};
static Wchar kurai3old[] = {
  0,
  0xbdbd,
  0xc9b4,
  0xc0e9,
