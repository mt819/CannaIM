...
nword2str(struct RkContext* cx, struct nword* w, Wchar* yomi)
{
  ...
  while (p-- > words) {
    ...
    ekanji = ustoeuc(kanji,
                     wp->nw_klen - wp->nw_left->nw_klen,
                     msg + msg_idx,
                     RK_LINE_BMAX - msg_idx);
    msg_idx = ekanji - msg;
    ustoeuc(yomi + wp->nw_left->nw_ylen,
            wp->nw_ylen - wp->nw_left->nw_ylen,
            eyomi,
            RK_LINE_BMAX);
         ...
         FILE* fp)
{
  ...
  for (i = from; i < end; i++, bun++) {
    ...
    if (log & 1) {
      ...
      l = l->next = (struct henkanlog*)malloc(sizeof(struct henkanlog));
      l->next = p;
    } else if (log == 2)
      l = l->next;
    ...
    if (!w) {
      if (log & 1)
        ...
        unsigned char *ekanji, *ustoeuc();
        ustoeuc(store->yomi + bun->nb_yoff, bun->nb_curlen, msg, RK_LINE_BMAX);
        fprintf(fp, "リテラル %s\n", msg);
      }
    } else {
...
allocBunStorage(unsigned len)
{
  ...
  if (s) {
    ...
    s->yomi =
      (Wchar*)calloc((s->maxyomi + 1 + 2 * OVERRUN_MARGIN), sizeof(Wchar));
    s->maxbunq = len;
    s->maxbun = (unsigned)0;
    s->curbun = 0;
    s->bunq = (struct nbun*)calloc((s->maxbunq + 1 + 2 * OVERRUN_MARGIN),
                                   sizeof(struct nbun));
    s->maxxq = len;
    s->xq = (struct nqueue*)calloc(
      (unsigned)(s->maxxq + 1 + 2 * OVERRUN_MARGIN), sizeof(struct nqueue));
    s->xqh = (struct nword**)calloc(
      (unsigned)(s->maxxq + 1 + 2 * OVERRUN_MARGIN), sizeof(struct nword*));
...
RkwBgnBun(int cx_num, Wchar* yomi, int n, int kouhomode)
{
  ...
  for (mask1 = (unsigned long)kouhomode, mask2 = 0L; mask1;
       mask1 >>= RK_XFERBITS) {
...
// int RkwSubstYomi(int /*cx_num*/, int /*ys*/, int /*ye*/, Wchar * /*yomi*/,
// int /*newLen*/);
...
RkwSubstYomi(int cx_num, int ys, int ye, Wchar* yomi, int newLen)
{
  ...
  if (!(store = cx->store) || !(bun = &store->bunq[store->maxbun]) ||
...
{
  struct RkContext* cx;
  if (!(cx = RkGetContext(cx_num)) || !IS_XFERCTX(cx) || !IS_XAUTCTX(cx)) {
    RkSetErrno(RK_ERRNO_ECTXNO);
    return (-1);
  }
...
RkwGoTo(int cx_num, int bnum)
{
  ...
  if (!(cx = RkGetXContext(cx_num)) || !(store = cx->store) || !store->maxbun) {
    RkSetErrno(RK_ERRNO_ECTXNO);
    return 0;
  }
...
getXFER(struct RkContext* cx, int cnum)
{
  ...
  return (cnum < 0 ? RK_NFER
                   : (cx->kouhomode >> (RK_XFERBITS * cnum)) & RK_XFERMASK);
}
...
RkwXfer(int cx_num, int knum)
{
  ...
  if (!(cx = RkGetXContext(cx_num)) || !(cx->store) ||
      !(bun = getCurrentBun(cx->store))) {
    RkSetErrno(RK_ERRNO_ECTXNO);
    return 0;
  }
...
findBranch(struct nstore* store, int cnum)
{
  ...
  if (!(bun = getCurrentBun(store)) || (0 > cnum) ||
      (cnum >= (int)bun->nb_maxcand))
    return NULL;
  for (w = bun->nb_cand; w; w = w->nw_next) {
    if (CanSplitWord(w) && bun->nb_curlen == w->nw_ylen) {
...
RkwGetStat(int cx_num, RkStat* st)
{
  ...
  if (!(cx = RkGetXContext(cx_num)) || !(store = cx->store) ||
      !(bun = getCurrentBun(store)) || !st) {
    RkSetErrno(RK_ERRNO_ECTXNO);
    return -1;
  }
  /* set up void values */
  st->bunnum = st->candnum = st->maxcand = st->diccand = st->ylen = st->klen =
    st->tlen = 0;
...
static int
getIt(struct RkContext* cx,
      int cnum,
      // FIXME prototype int (*proc)(Wchar *, int, int, Wchar *, Wchar *, RkLex
      // *, struct RkContext *),
      int (*proc)(),
      Wchar* dst,
      int max)
{
  ...
  if (!(bun = getCurrentBun(store)) || !(w = findBranch(store, cnum)))
    return (-1);
  return addIt(w,
...
RkwQueryDic(int cx_num, char* dirname, char* dicname, struct DicInfo* status)
{
  ...
  if (!(cx = RkGetContext(cx_num)) || !status || !dirname || !dicname ||
      !dicname[0])
    return (-1);
  ...
  if (*dirname && strcmp(dirname, cx->ddpath[0]->dd_name) &&
    ...
    if (RkwSetDicPath(new_cx_num, dirname) < 0) {
      CloseContext(new_cx_num) free(buff);
      return NOTALC;
    }
  ...
  if (df) {
    ...
    if (file) {
      ...
      status->di_mode = 0;
      CloseContext(new_cx_num) free(file);
      free(buff);
      return (0);
    }
  }
  free(buff);
  CloseContext(new_cx_num) return (-1);
}
