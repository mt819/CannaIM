...
/* ARGSUSED */
{
  ...
  if (!(df = dm->dm_file) || !(dd = df->df_direct))
    return -1;
  if (!df->df_rcount) {
    if ((df->df_fdes = (long)openDF(df, dfnm, &writable, &gramoff, &gramsz)) <
        0)
      return -1;
    if (writable)
...
assurep(struct ND* dic, int id)
{
  ...
  if (!isLoadedPage(dic->pgs + id)) {
    ...
    buf = (unsigned char*)mmap(
      0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd_dic, 0);
    if ((int)buf == -1)
      return NULL;
#else
  ...
  if (xdm->pgs[pgno].buf) {
    ...
    if (_RkEql(key + cur, wrec1, remlen)) {
      if (remlen + cur > ylen)
        ...
        if (nread[nc].cache) {
          if (_RkGetLink(xdm, pgno, val, &nread[nc].offset, &nread[nc].csn) <
              0) {
             ...
             int* cf)
{
  ...
  if (!*pos && !*(pos + 1)) {
    ...
    nc = readThisCache(dm, xdm, pgno, val, key, cur, ylen, nread, mc, nc, cf);
    if (*(pos + 2) & LAST_NODE)
      return (nc);
    pos += 4;
  }
            ...
            int* cf)
{
  ...
  if (iw) {
    ...
    if (val < xdm->drsz)
      nc =
        SearchInDir(dm, xdm, xdm->buf + val, key, cur, ylen, nread, mc, nc, cf);
    else {
      pgno = (val - xdm->drsz) / xdm->pgsz;
      val -= pgno * xdm->pgsz + xdm->drsz;
      p = assurep(xdm, pgno);
      if (p)
        nc = SearchInPage(
          dm, xdm, pgno, p, val, key, cur, ylen, nread, mc, nc, cf);
    }
  }
  return (nc);
}
...
int
_Rkpsearch(struct RkContext* cx,
           struct DM* dm,
           Wchar* key,
           int n,
           struct nread* nread,
    ...
    if (RkParseOWrec(gram, arg, wrec, PERM_WRECSIZE, lucks)) {
      ...
      *(key + ylen) = 0;
      ...
      if (xdm) {
        if (xdm->buf)
          nc = SearchInDir(
            dm, xdm, xdm->buf, key, 0, ylen, nread, maxcache, 0, &cf);
      }
