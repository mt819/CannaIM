...
static void
clearWord(struct nword* w, int bb) /* make word empty */
{
  if (w) {
    w->nw_cache = NULL;
    w->nw_rowcol = bb; /* 文節 */
...
static void
derefWord(struct nword* word) /* decrease the reference counter */
{
...
static void
freeWord(struct nstore* st,
         struct nword* word) /* freeWord = derefWord + killWord */
{
  derefWord(word);
  killWord(st, word);
}
...
void
_RkFreeBunq(struct nstore* st) /* freeWord = derefWord + killWord */
{
...
static struct nword* concWord(cx,
                              ...
                              q,
                              loc,
                              bb) /* create the concatinated word p+q */
  struct RkContext* cx;
...
int bb;
{
  ...
  if (conc.nw_klen > RK_LEN_WMAX || conc.nw_ylen > RK_LEN_WMAX ||
      conc.nw_count >= RK_CONC_NMAX)
    return NULL;
#ifdef LOGIC_HACK
  if (conc.nw_count >= 3) {
    switch (RkCheckNegGram(
      cx->gram->gramdic, p->nw_left->nw_rowcol, p->nw_rowcol, q->nw_rowcol)) {
      case 1:
        return NULL;
      case 2:
...
cvtAlpha(Wchar* dst, int maxdst, Wchar* src, int maxsrc, int format)
{
  switch (format) {
    ...
    case 1:
      return RkwCvtZen(dst, maxdst, src, maxsrc);
    case 2:
      return RkwCvtHan(dst, maxdst, src, maxsrc);
    case 3:
      return -1;
#else
    case 1:
...
cvtHira(Wchar* dst, int maxdst, Wchar* src, int maxsrc, int format)
{
  switch (format) {
    case 1:
      return RkwCvtHira(dst, maxdst, src, maxsrc);
    case 2:
      return RkwCvtKana(dst, maxdst, src, maxsrc);
    default:
      return 0;
  }
}
  ...
  switch (format >> 4) {
    case LIT_NUM:
      if (mode & RK_MAKE_KANSUUJI)
        ...
        return RkwCvtNone(dst, maxdst, src, maxsrc);
    case LIT_ALPHA:
      return cvtAlpha(dst, maxdst, src, maxsrc, format & 15);
    case LIT_HIRA:
      return cvtHira(dst, maxdst, src, maxsrc, format & 15);
    default:
      return 0;
  }
}
       ...
       int format)
{
  ...
  for (mode = cx->litmode[format]; mode; mode >>= RK_XFERBITS)
    if (w < word + maxword) {
      ...
      dstlen =
        cvtLit((Wchar*)0, 9999, src, srclen, code, (unsigned long)cx->concmode);
         ...
         int douniq)
{
  ...
  candidates = (unsigned char**)malloc(sizeof(unsigned char*) * RK_CAND_NMAX);
  ...
  for (md = head->md_next; md != head; md = md->md_next) {
    ...
    for (c = 0; c < nc; c++) {
      ...
      if (ys < thisRead->nk && thisRead->nk <= ye &&
          thisRead->nk <= RK_KEY_WMAX) {
  ...
  if (us_iscodeG0(c)) {         /* ascii string */
    ...
    if (0xb000 <= c) { /* kanji string */
      ...
      if (!(cx->concmode & RK_MAKE_EISUUJI)) {
        ...
        for (; k < z; k++, clen++)
          if (clen >= RK_KEY_WMAX || !((0xa3c1 <= (c = *k) && c <= 0xa3da) ||
                                       (0xa3e1 <= c && c <= 0xa3fa))) {
            doflush++;
            break;
          }
...
int douniq;
{
  ...
  for (clen = 0; (clen <= maxclen && clen < ye); clen++) {
    ...
    ye1 = ye - clen;
    r = readWord(
      cx, yy + clen, ys1, ye1, class, right, RIGHTSIZE - 1, doflush, douniq);
    if (Is_Word_Make(cx))
      r = makeWord(cx,
                   yy + clen,
                   ys1,
 ...
 */
Wchar* _RkGetKanji(cw, key, mode) unsigned long mode;
struct nword* cw;
Wchar* key;
{
  Wrec* str;
...
uniqWord(Wchar* key, struct nword* words, unsigned ylen, unsigned long mode)
{
  ...
  if (!(!key || !ylen)) {
    ...
    uniq[0] = uniq[1] = uniq[2] = uniq[3] = uniq[4] = uniq[5] = uniq[6] =
      uniq[7] = uniq[8] = uniq[9] = uniq[10] = uniq[11] = uniq[12] = uniq[13] =
        uniq[14] = uniq[15] = -1;
...
static struct nword* sortWord(words) struct nword* words;
{
...
list2height(struct nword* height[], int maxclen, struct nword* parse)
{
  ...
  for (p = parse; p; p = p->nw_next)
    if ((unsigned long)p->nw_ylen <= (unsigned long)maxclen &&
        !height[p->nw_ylen])
      height[p->nw_ylen] = p;
  for (i = 0; i <= maxclen; i++)
    if (height[i]) {
 ...
 */
static struct nword*
  parseBun(cx, yy, ys, ye, doflush, douniq, maxclen) struct RkContext* cx;
...
#else
  struct splitParm* ul2 =
    (struct splitParm*)malloc(sizeof(struct splitParm) * PARMSIZE);
  if (!ul2) {
    return L1;
  }
#endif
...
doLearn(struct RkContext* cx, struct nword* thisW)
{
  ...
  unsigned* permutation;
  candidates = (unsigned char**)calloc(RK_CAND_NMAX, sizeof(unsigned char*));
  permutation = (unsigned*)malloc(sizeof(unsigned) * RK_CAND_NMAX);
...
_RkLearnBun(struct RkContext* cx, int cur, int mode)
{
  ...
  if (mode) {
    if (bun->nb_flags & RK_REARRANGED) {
      ylen =
        bun->nb_curlen + (cur < (int)st->maxbun - 1 ? (bun + 1)->nb_curlen : 0);
      pos = bun->nb_curlen;
      if (ylen < 32) {
